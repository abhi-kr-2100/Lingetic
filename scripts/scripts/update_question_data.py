    ...
    try:
        with conn.cursor() as cur:
            valid = [
                ...
                for q in questions
                if q.get("id") and q.get("question_type_specific_data") is not None
            ]
            if not valid:
            ...
            conn.commit()
            print(f"Successfully updated {len(valid)} questions' question_type_specific_data in the database.")
    except Exception as e:
        conn.rollback()
