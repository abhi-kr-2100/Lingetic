...
def make_gemini_api_call(prompt: str, entry: dict) -> dict:
    ...
    model_name = (
        "gemini-1.5-flash-8b"  # Using a specific model known to work well
    )
...
def main(language: str, schema: str, output: str):
    ...
    results = []
    failed_entries_indices = []  # Keep track of failed entry indices
    total_entries = len(schema_entries)
    print(
        f"Generating content for {total_entries} entries...",
        file=sys.stderr,
    )
    for i, entry in enumerate(schema_entries):
        print(
            f"Processing entry {i + 1}/{total_entries}: Theme='{entry.get('theme', '')}', Level='{entry.get('level', '')}'",
            file=sys.stderr,
        )
        try:
            # get_sentences_for_entry now calls the modified make_gemini_api_call
            # which raises exceptions on failure (including max retries)
            res = get_sentences_for_entry(language, entry)

            # Basic check if the result seems valid (has 'data' key which is a list)
            # More robust validation happens via Pydantic within make_gemini_api_call
            if "data" not in res or not isinstance(res["data"], list):
                print(
                    f"Warning: Entry {i + 1} result missing or invalid 'data' key. Result: {res}",
                    file=sys.stderr,
                )
                failed_entries_indices.append(i)  # Record failure index
                print(
                    f"Skipping entry {i + 1} due to unexpected result structure.",
                    file=sys.stderr,
                )
            ...
            # Optional: Add a small fixed delay even after successful calls to be conservative
            # time.sleep(1) # e.g., wait 1 second between successful calls

        except (SystemExit, KeyboardInterrupt):  # Allow termination signals
            print("\nTermination signal received. Exiting.", file=sys.stderr)
            # Optionally save progress or perform cleanup here if needed
            sys.exit(1)  # Ensure script exits
        except Exception as e:
            # Catch any exception raised by get_sentences_for_entry or make_gemini_api_call
            print(f"Error processing entry {i + 1}: {e}", file=sys.stderr)
            failed_entries_indices.append(
                i
            )  # Record the index of the failed entry
            print(f"Skipping entry {i + 1} due to error.", file=sys.stderr)
            # Continue to the next iteration of the loop

    # --- Reporting Phase ---
    if failed_entries_indices:
        print("\n--- Summary of Failed Entries ---", file=sys.stderr)
        for index in failed_entries_indices:
            # Retrieve the original entry data for context
            failed_entry = schema_entries[index]
            theme = failed_entry.get("theme", "N/A")
            level = failed_entry.get("level", "N/A")
            print(
                f"- Entry {index + 1}: Theme='{theme}', Level='{level}'",
                file=sys.stderr,
            )
        print("---------------------------------", file=sys.stderr)
    else:
        # Only print this if there were entries to process
        if total_entries > 0:
            print("\nAll entries processed successfully.", file=sys.stderr)
    ...
    if not results:
        ...
        try:
            ...
            # Decide if this error should cause script exit
        finally:
            if needs_closing:
                output_file.close()
